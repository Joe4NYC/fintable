...
當 `apps-script/Code.gs` 有更新（例如新增「資產快照」「多裝置寫入保護」等功能），要重新部署才會生效：

1. Google Sheet → **擴充功能 → Apps Script**，把 `apps-script/Code.gs` 整份覆蓋貼上。
   **保留你原本那行 `SECRET`**（不要被預設值蓋掉）。儲存。
2. 右上 **部署 → 管理部署作業 → 編輯（鉛筆圖示）→ 版本選「新版本」→ 部署**。

> ⚠️ 用「**管理部署作業 → 新版本**」更新現有部署，`/exec` 網址**不會變**，網站不用重新連接。
> 若改按「**新增部署作業**」會產生**新網址**，那就得到網站「設定」頁重新貼上新網址連接。

更新後下次在網站存檔，會自動建立「資產快照」分頁並開始記錄；累積數筆後，儀表板的淨資產走勢圖就會改用真實資料。
